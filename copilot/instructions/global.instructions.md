...
リポジトリ直下の00-で始まるディレクトリはグローバルでignoreしているローカルディレクトリである。そこには構想段階のメモや内部レビュー資料を配置するが、そのディレクトリ内のファイルやファイルの内容、ディレクトリそのものの存在をローカルおよびリモートのリポジトリ、ISSUE/PRなどに決して漏出させてはならない。根拠として引用するのも禁止。
