...
if [ -f "package.json" ]; then
    if grep -q '"vitest"' package.json 2>/dev/null; then
        OUTPUT=$(npx --no-install vitest run --coverage --reporter=verbose 2>/dev/null | grep -E "Statements|Branches|Functions|Lines|All files" || true)
        ...
        OUTPUT=$(npx --no-install jest --coverage --coverageReporters=text-summary 2>/dev/null | grep -E "Statements|Branches|Functions|Lines" || true)
    ...
    if go test -coverprofile=coverage.out ./... 2>/dev/null; then
        ...
        echo "Run: go test ./... (build or tests failed)"
    fi
    rm -f coverage.out
    ...
    COVERAGE=$(echo "$OUTPUT" | grep -E "^TOTAL" | grep -oE '[0-9]+(\.[0-9]+)?%' | tr -d '%' | head -1 || true)
